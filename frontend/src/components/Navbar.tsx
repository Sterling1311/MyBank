    ...
    <nav className="bg-[#156064] text-white">
      ...
      {isOpen && (
        <div className="md:hidden bg-[#0e4547] px-6 py-4 flex flex-col gap-4 border-t border-[#00C49A]">
          <Link to="/dashboard" onClick={() => setIsOpen(false)} className="hover:text-[#F8E16C] transition-colors py-2">
            Dashboard
          </Link>
          <Link to="/budget" onClick={() => setIsOpen(false)} className="hover:text-[#F8E16C] transition-colors py-2">
            Budget
          </Link>
          <Link to="/categories" onClick={() => setIsOpen(false)} className="hover:text-[#F8E16C] transition-colors py-2">
            Categories
          </Link>
          <button
            onClick={() => { logout(); setIsOpen(false); }}
            className="bg-[#00C49A] px-4 py-2 rounded-lg hover:bg-white hover:text-[#156064] transition-colors text-left"
          >
            Logout
          </button>
        </div>
